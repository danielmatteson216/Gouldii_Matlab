...
VX1_settle = VX1_settle(Serial_startdate:Serial_enddate, :);
VX2_settle = VX2_settle(Serial_startdate:Serial_enddate, :);
...
%XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
    %nig rig for contracts pre....
        ...
        %elseif (TradeDay(i) == 2) || (TradeDay(i) == 3 && TradeDay(i-1) == 6) || (TradeDay(i) == 5 && nexttradeday == 2) 
           elseif (TradeDay(i) == 2 && cashonweekendsflag == 1 && ((frisig_vx1(i-1,1)+frisig_vx2(i-1,1)<0))) || (TradeDay(i) == 3 && TradeDay(i-1) == 6 && cashonweekendsflag == 1 && ((frisig_vx1(i-1,1)+frisig_vx2(i-1,1)<0)))
            %elseif TradeDay(i) == 2          
...
%---------------------------------------------------------------------------
            ...
            %   STOP LOSS --- is it triggered?
              ...
              % can be active and not triggered -- or not active at all        
                            ...
                            if (TradeDay(i) == 6 && cashonweekendsflag == 1 && (sigw1(i,1)+ sigw2(i,1) < 0)) || (TradeDay(i) == 5 && nexttradeday == 2 && cashonweekendsflag == 1 && (sigw1(i,1)+ sigw2(i,1) < 0))         % this happens on friday ---- if the holiday is on friday
...
%---------------------------------------------------------------------------
                            ...
                            TradeVX2Target(i,1) = (TargetWeightVX2postsig(i,1) - ExposureVX2Pre(i,1)) * PortfolioNetLiqPre(i,1);
                                    ...
                                    if TradeVX1Contracts(i,1) <= 0
                                       TradeVX1Actual(i,1) = ((VX1_settle(i) - .025) * 1000 * TradeVX1Contracts(i,1)) + Commission*TradeVX1Contracts(i,1);
                                    elseif TradeVX1Contracts(i,1) > 0
                                       TradeVX1Actual(i,1) = ((VX1_settle(i) + .025) * 1000 * TradeVX1Contracts(i,1)) - Commission*TradeVX1Contracts(i,1);
                                   end 
                                    
                                    if TradeVX2Contracts(i,1) <= 0
                                       TradeVX2Actual(i,1) = ((VX2_settle(i) - .025) * 1000 * TradeVX2Contracts(i,1)) + Commission*TradeVX2Contracts(i,1);                                        
